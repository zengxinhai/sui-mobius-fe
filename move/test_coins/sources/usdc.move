module test_coins::usdc {
  ...
  fun init(wit: USDC, ctx: &mut TxContext) {
    let decimals: u8 = 9;
    let symbol: vector<u8> = b"USDC";
    let name: vector<u8> = b"USDCoin";
    ...
    coin::mint_and_transfer(&mut treasuryCap, math::pow(10, decimals + 8), sender, ctx);
